...
export function SectorHeatmap({ news }: Props) {
  const sectorData = SECTORS.map(sector => {
    const sectorNews = news.filter(n => n.affectedSectors.includes(sector));
    ...
    <div className="border border-border bg-card rounded-lg p-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        {sectorData.map(({ sector, count, score }) => (
          <div key={sector} className={`border rounded px-3 py-2 ${getColor(score)} transition-all`}>
            <div className="text-xs font-mono font-bold">{sector}</div>
            <div className="text-[10px] font-mono opacity-70">{count} article{count > 1 ? 's' : ''} · {score > 0 ? '+' : ''}{score}</div>
          </div>
        ))}
      </div>
    </div>
